...
public class EUExBaiduNavi extends EUExBase {
	...
	public void init(String[] params) {
		...
		MLog.getIns().i("mSDCardPath = " + mSDCardPath);
		...
		BaiduNaviManager.getInstance().init((Activity) mContext, mSDCardPath, Constant.APP_FOLDER_NAME, new NaviInitListener() {
			...
			public void initSuccess() {
				...
				if (isKeyCorrect) {
					cbInit(true);// 只有当key配置正确且引擎也初始化完成后才回调cbInit = true
				} else {
					MLog.getIns().i("key校验失败, " + keyErrMsg);
	 ...
	 * @param flag
	 */
	private void cbInit(boolean flag) {
	...
	public void startRoutePlan(String[] params) {
		...
		try {
			...
			e.getStackTrace();
			MLog.getIns().e(e);
			cbStartRoutePlan(RESULT_CODE_START_ROUTE_PLAN_FAIL, ERROR_INFO_START_ROUTE_PLAN_5);
			return;
		}
		...
		cbStartRoutePlan(RESULT_CODE_START_ROUTE_PLAN_SUCCESS, 0);
	}
	 ...
	 * @param flag
	 */
	private void cbStartRoutePlan(int resultCode, int errorInfo) {
